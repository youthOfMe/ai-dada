...
import com.yang.yangdada.mapper.ScoringResultMapper;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult>
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            // 补充校验规则
            ThrowUtils.throwIf(StringUtils.isBlank(resultName), ErrorCode.PARAMS_ERROR, "结果名称不可为空");
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "ID规则违规");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultDesc.length() > 128, ErrorCode.PARAMS_ERROR, "结果名称长度不能超过128");
        }
        // 补充校验规则
        if (appId != null) {
